...
(define (extend-key key size)
  (let keypatch ([cheie key] [dimensiune size])
    (cond [(= (length cheie) dimensiune) cheie]
          [(> (length cheie) dimensiune) (keypatch (reverse (cdr (reverse cheie))) dimensiune)]
          [else (keypatch (append cheie cheie) dimensiune)]
    )))
...
(define (encrypt-decrypt func code key)
  (map func code key))
  

(define (encrypt-codes message key)
  (encrypt-decrypt (λ(x y) (modulo (+ x y) 27)) message (extend-key key (length message))))
(define (decrypt-codes crypted key)
  (encrypt-decrypt (λ(x y) (modulo (- x y) 27)) crypted (extend-key key (length crypted))))
